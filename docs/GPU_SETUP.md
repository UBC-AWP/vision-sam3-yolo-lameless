...
GPU mode is selected via `COMPOSE_PROFILES=gpu` with `docker-compose.yml`.
...
4. **View logs**:
   ```bash
  docker compose logs -f video-preprocessing
   ```
...
3. Verify container has GPU access:
   ```bash
   docker compose exec video-preprocessing nvidia-smi
   ```
...
2. **Development Workflow**
   - Build and test locally with `COMPOSE_PROFILES=gpu docker compose ...`
