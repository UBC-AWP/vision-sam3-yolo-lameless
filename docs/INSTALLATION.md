...
| `COMPOSE_PROFILES` | (auto) | `gpu` when available, otherwise `cpu` |
...
**Long-video preprocessing (required for segmentation + bad-video detection):**

- Model files must exist in `services/video-preprocessing/models/`:
  - `xgboost_model.pkl`
  - `feature_scaler.pkl`
  - `feature_info.json`
- Set these optional environment variables if needed:
  - `LONG_VIDEO_YOLO_MODEL` (default `yolov8l.pt`)
  - `LONG_VIDEO_YOLO_CONF` (default `0.3`)
  - `LONG_VIDEO_YOLO_IOU` (default `0.3`)
  - `LONG_VIDEO_BAD_THRESHOLD` (override model threshold)
...
`deploy.sh` now auto-selects preprocessing profile:
- Uses `gpu` when `nvidia-smi` is available
- Falls back to `cpu` when no NVIDIA GPU is detected
...
# 2. Start services (GPU profile by default, CPU fallback if needed)
COMPOSE_PROFILES=gpu docker compose up -d
# OR
COMPOSE_PROFILES=cpu docker compose up -d

# 3. Start infrastructure services first (if you want staged startup)
...
# 7. (Optional) Start all remaining services if doing staged startup
docker compose up -d
```
