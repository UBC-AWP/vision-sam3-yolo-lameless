...
DEFAULT_MODEL_PATH = os.getenv("LONG_VIDEO_SHORT_YOLO_MODEL") or os.getenv(
    "LONG_VIDEO_YOLO_MODEL", "yolov8l.pt"
)
CONFIDENCE_THRESHOLD = float(os.getenv("LONG_VIDEO_SHORT_YOLO_CONF", "0.2"))
IOU_THRESHOLD = float(os.getenv("LONG_VIDEO_SHORT_YOLO_IOU", "0.3"))
...
def process_video_to_csv(video_path: str, output_csv_path: str, model: YOLO | None = None) -> bool:
    print(f"Processing short video: {os.path.basename(video_path)}")
    ...
    while True:
        ...
        for human in detections["humans"]:
            ...
            csv_data.append([frame_count, f"person {conf_rounded}", conf, x1, y1, x2, y2])

        for cow in detections["cows"]:
            x1, y1, x2, y2 = cow["box"]
            conf = cow["confidence"]
            conf_rounded = round(conf, 2)
            csv_data.append([frame_count, f"cow {conf_rounded}", conf, x1, y1, x2, y2])
