...
class User(Base):
    ...
    __table_args__ = (
        CheckConstraint("role IN ('admin', 'tenant_admin', 'researcher', 'viewer', 'rater')", name="valid_role"),
    )
...
class ClipVideo(Base):
    """Clip (short) video derived from a raw video"""
    __tablename__ = "clip_videos"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False, index=True)
    raw_video_id = Column(String(36), ForeignKey("videos.id"), nullable=False, index=True)
    clip_slug = Column(String(20), nullable=False)  # e.g. clip_1, clip_2
    ...
    label = Column(String(10), nullable=True)  # good, bad
    prob_bad = Column(Float, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)

    __table_args__ = (
        Index("idx_clip_videos_tenant", "tenant_id"),
        Index("idx_clip_videos_raw", "raw_video_id"),
        Index("uq_clip_videos_raw_slug", "raw_video_id", "clip_slug", unique=True),
    )
