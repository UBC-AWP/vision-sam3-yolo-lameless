...
export const videosApi = {
  upload: async (file: File, label?: number, onProgress?: (progress: number) => void) => {
    ...
    // Explicit filename so backend always receives original name (some proxies/clients omit it)
  ...
  importFromUrl: async (videoUrl: string, label?: number, metadata?: Record<string, unknown>) => {
    const response = await apiClient.post('/api/videos/import-url', {
      video_url: videoUrl,
      label,
      metadata,
    })
  ...
  list: async (
    skip = 0,
    limit = 100,
    filters?: { label?: number; status?: string }
  ) => {
...
// Storage endpoints for raw/short videos
export const storageApi = {
  listRawVideos: async (includeUrl = false, tenantId?: string) => {
    const params: { include_url: boolean; tenant_id?: string } = { include_url: includeUrl }
    if (tenantId) {
      params.tenant_id = tenantId
    }
    const response = await apiClient.get('/api/videos/storage/raw-videos', { params })
    return response.data
  },
  listShortVideos: async (includeUrl = false, tenantId?: string) => {
    const params: { include_url: boolean; tenant_id?: string } = { include_url: includeUrl }
    if (tenantId) {
      params.tenant_id = tenantId
    }
    const response = await apiClient.get('/api/videos/storage/clips', { params })
    return response.data
  },
  getShortVideoUrl: async (key: string): Promise<{ url: string; key: string }> => {
    const response = await apiClient.get('/api/videos/storage/clips/url', {
      params: { key }
    })
    return response.data
  },
  processRawVideos: async (keys: string[]) => {
    const response = await apiClient.post('/api/videos/storage/raw-videos/process', { keys })
    return response.data
  },
}
...
export interface User {
  ...
  email: string
  username: string
  role: 'admin' | 'tenant_admin' | 'researcher' | 'viewer' | 'rater'
  is_active: boolean
