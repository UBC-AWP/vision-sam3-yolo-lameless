...
CREATE TABLE IF NOT EXISTS users (
    ...
    CONSTRAINT valid_role CHECK (role IN ('admin', 'tenant_admin', 'researcher', 'viewer', 'rater'))
);
...
CREATE TABLE IF NOT EXISTS videos (
    ...
    label INTEGER,
    label_confidence VARCHAR(20),
    status VARCHAR(20) DEFAULT 'uploaded',
    has_analysis BOOLEAN DEFAULT FALSE,
    has_annotated BOOLEAN DEFAULT FALSE,
    duration FLOAT,
    fps FLOAT,
    width INTEGER,
    height INTEGER,
...
CREATE INDEX IF NOT EXISTS idx_videos_storage ON videos(storage_backend);
...
-- ============== CLIP VIDEOS (short clips from raw) ==============
...
BEGIN
    ...
    -- Add tenant_id to videos if missing
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
...
VALUES (
    ...
    TRUE,
    CURRENT_TIMESTAMP
) ON CONFLICT (name) DO UPDATE SET created_at = COALESCE(tenants.created_at, CURRENT_TIMESTAMP);
...
INSERT INTO users (id, email, username, password_hash, role, is_active, rater_tier, tenant_id, created_at)
VALUES (
