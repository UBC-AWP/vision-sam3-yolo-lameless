#!/usr/bin/env python3
"""
One-time migration: rename S3 objects from short/ to clips/ and update DB.

Old path: tenants/{tenant_id}/short/{raw_video_id}/{clip_slug}.mp4
New path: tenants/{tenant_id}/clips/{raw_video_id}/{clip_slug}.mp4
...
def main():
    ...
    print("Scanning S3 for objects in tenants/*/short/...")
    
    # Find all objects in tenants/{tenant_id}/short/ paths
    ...
    for page in paginator.paginate(Bucket=BUCKET, Prefix=OLD_PREFIX):
        for obj in page.get("Contents", []):
            ...
            if not key or key.endswith("/"):
                continue
            # Check if it's in the old short/ path
            if OLD_SUBPATH in key:
                objects_to_migrate.append(key)
    
    if not objects_to_migrate:
        print("No objects found in tenants/*/short/ paths. Nothing to migrate.")
        conn.close()
        return
    ...
    for old_key in objects_to_migrate:
        # Replace /short/ with /clips/
        new_key = old_key.replace(OLD_SUBPATH, NEW_SUBPATH)
        
        # Check if new key already exists
        try:
            s3.head_object(Bucket=BUCKET, Key=new_key)
            print(f"  Skip {old_key}: destination already exists")
